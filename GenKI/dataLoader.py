...
class scBase():
    def __init__(self, 
        ...
        check_adata(adata)
        self._gene_names = list(adata.var_names)
        self.device = device  # Use the defined device

        # Validate target genes
        if isinstance(target_gene[0], int):
            target_gene = adata.var_names[target_gene].tolist()
        if all([g not in self._gene_names for g in target_gene]):
            raise IndexError("The input target gene should be in the gene list of adata")
        else:
        ...
        if target_cell is None:
            ...
            self._counts = adata[adata.obs[obs_label] == target_cell, :].X
        self._counts = scipy.sparse.lil_matrix(self._counts)
        ...
        if rebuild_GRN:
            ...
            scipy.sparse.save_npz(pcNet_path, self._net)
            if verbose:
                print(f"GRN has been built and saved in \"{pcNet_path}\"")
        else:
            if verbose:
    ...
    def __call__(self, gene_name: list[str]):
        return [self._gene_names.index(g) for g in gene_name]
...
class DataLoader(scBase):
    def __init__(self, 
        ...
        self.verbose = verbose
        self.cutoff = cutoff
        self.edge_index = torch.tensor(self._build_edges(), dtype=torch.long).to(self.device)
    ...
    def _gen_ZINB(self, 
        ...
        s_BIN = torch.bernoulli(torch.full((self._counts.shape[0],), p_BIN, device=self.device))

        s = torch.stack([
            torch.distributions.NegativeBinomial(
                total_count=decay * n_NB, 
                probs=p_NB
            ).sample((self._counts.shape[0],)).to(self.device) * s_BIN
            for decay in decays
        ], dim=1)

        if noise:
            noise_tensor = torch.randint(-1, 2, size=s[s > 0].shape, device=self.device)
            noise_probs = torch.tensor([0.15, 0.7, 0.15], device=self.device)
            noise_choices = torch.multinomial(noise_probs, len(noise_tensor), replacement=True)
            s[s > 0] += noise_choices - 1

        if normalize:
            scale_factor = 10e4
            s = torch.log1p(s * scale_factor / s.sum(dim=0, keepdim=True))

        if show:
            fig, ax = plt.subplots(ncols=2, figsize=(12, 5))
            counts = torch.tensor(self._counts.toarray(), dtype=torch.float32).to(self.device)
            for i, (values, title) in enumerate(zip([counts[:, self(self._target_gene)], s], ['WT', 'Simulated'])):
                ax[i].hist(values.cpu().numpy(), bins=counts.shape[0] // 5, label=self._target_gene)
                ax[i].set_title(title)
                ax[i].legend()
            plt.show()
    ...
    def load_data(self):
        counts = torch.tensor(self._counts.toarray().T, dtype=torch.float32).to(self.device)
        return Data(x=counts, edge_index=self.edge_index, y=self._gene_names)

    def load_kodata(self):
        mask = ~(torch.isin(self.edge_index[0], torch.tensor(self(self._target_gene), device=self.device)) + 
                 torch.isin(self.edge_index[1], torch.tensor(self(self._target_gene), device=self.device)))
        edge_index_KO = self.edge_index[:, mask]

        counts_KO = self._counts.copy()
        counts_KO[:, self(self._target_gene)] = 0
        counts_KO = torch.tensor(counts_KO.toarray().T, dtype=torch.float32).to(self.device)

        return Data(x=counts_KO, edge_index=edge_index_KO, y=self._gene_names)
